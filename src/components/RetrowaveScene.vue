 ...
 */
RetrowaveScene.prototype.prepareScene = function (wantProceduralSky = false, wantAnimation = false) {
...
RetrowaveScene.prototype.setPostProcessing = function () {
	...
	this.bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 1.5, 0, 0.8) // Settings here appear to have no effect
...
RetrowaveScene.prototype.addRoadLines = function () {
	...
	let roadLineCenterLeftGeometry = new THREE.PlaneBufferGeometry(0.15, 300, 0, 0)
	roadLineCenterLeftGeometry.translate(-1.8, 110, 0.2)
	roadLineCenterLeftGeometry.rotateX(-Math.PI * 0.5)

	let roadLineCenterRightGeometry = new THREE.PlaneBufferGeometry(0.15, 300, 0, 0)
	...
	let roadLinesGeometry = BufferGeometryUtils.mergeBufferGeometries(roadLinesConception, false) // Merge all geometries within sidewalkConception array
...
RetrowaveScene.prototype.addSidewalk = function () {
	...
	let sidewalkGeometry = BufferGeometryUtils.mergeBufferGeometries(sidewalkConception, false) // Merge all geometries within sidewalkConception array
...
RetrowaveScene.prototype.addGroupedPyramids = function () {
	...
	for (let i = 0; i < 80; i++) {
		...
		function checkPyramidHistory() {
			let checkHistory = this.checkPositionHistory(randomSize, translateX, translateZ)
		...
		if (ignoreGeometry == false) {
			pyramidGeometry = new THREE.ConeBufferGeometry(randomSize, randomSize, 4, 1, true, rotatePyramid)
	...
	let pyramidGroupGeometry = BufferGeometryUtils.mergeBufferGeometries(this.pyramidGroupConception, false) // Merge all geometries within sidewalkConception array
	...
	this.pyramidGroups = SceneUtils.createMultiMaterialObject(pyramidGroupInstance, pyramidGroupMaterials)
...
RetrowaveScene.prototype.addPalmtrees = function () {
	...
	// Merge (log + leaves)
	var palmTree = BufferGeometryUtils.mergeBufferGeometries(palmTreeConception, false)
...
RetrowaveScene.prototype.checkPositionHistory = function (sizeOfGeometry, currentPositionX, currentPositionZ) {
	...
	for (let i = 0; i < this.positionHistory.length; i++) {
		if (
			...
			currentPositionX + currentSize > this.positionHistory[i].positionX - this.positionHistory[i].size &&
			currentPositionZ + currentSize > this.positionHistory[i].positionZ - this.positionHistory[i].size &&
			currentPositionX - currentSize < this.positionHistory[i].positionX + this.positionHistory[i].size &&
			currentPositionZ - currentSize < this.positionHistory[i].positionZ + this.positionHistory[i].size
		) {
...
RetrowaveScene.prototype.addProceduralSky = function () {
	...
	this.starsGeometry.setAttribute('position', new THREE.Float32BufferAttribute(this.vertices, 3))
	...
	for (let i = 10; i < 20; i++) {
		this.stars = new THREE.Points(this.starsGeometry, this.starsMaterials[i % 6])
...
RetrowaveScene.prototype.addControls = function () {
	...
	if (window.DeviceOrientationEvent) {
		window.addEventListener('deviceorientation', motionControls.bind(this), false)
	}
...
RetrowaveScene.prototype.randomize = function (min, max, setting) {
	...
	if (setting == 'float') {
		...
		if (randomResult == this.previousOddRandomInteger || randomResult == this.previousEvenRandomInteger) {
			if (setting == 1) {
				if (
